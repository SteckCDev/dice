...
class Messages:
    ...
    def admin(users_since_launch: int) -> str:
        return f"{bold('🎲 Дайс / Админ-панель')}\n\n" \
               ...
               f"ставка (минимальная, максимальная), игроки (отмена, завершение) чат (раунды, отмена, завершение), " \
               f"фейк (периодичность, минимальная, максимальная), подписки (добавить, удалить)\n\n" \
